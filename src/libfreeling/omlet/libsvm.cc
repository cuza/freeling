...
#define strtok_r(p1, p2, p3) strtok_s(p1, p2, p3);
#endif 
...
svm_model *svm_load_model(const char *model_file_name)
{
  ...
  char *safe; // just to make strtok thread safe

  while(readline(fp,line,max_line_len)!=NULL)
    { 
      p = strtok_r(line, ":", &safe);
      while(1)
        {
          p = strtok_r(NULL,":", &safe);
          if(p == NULL)
            break;
          ++elements;
        }
  ...
  for(i=0;i<l;i++)
    {
      ...
      while(1)
        {
          idx = strtok_r(NULL, ":", &safe);
          val = strtok_r(NULL, " \t", &safe);
